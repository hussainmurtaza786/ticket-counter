...
import { trainSchema } from "../../Schemas/Index";
import { useDispatch, useSelector } from "react-redux";
import { addTransportTicketThunk } from "../../store/ticketSlice";

function Train({ selectedTransport, onClose }) {
    ...
    <div>
      <form onSubmit={handleSubmit}>
        <VStack spacing={4} align="stretch">
          ...
          <FormControl isInvalid={touched.depStation && errors.depStation}>
            <FormLabel>Departure Station</FormLabel>
            <Input
              type="text"
              name="depStation"
              value={values.depStation}
              onChange={handleChange}
              onBlur={handleBlur}
            />
            {touched.depStation && errors.depStation && (
              <Text color="red.500" fontSize="sm">
                {errors.depStation}
              </Text>
            )}
          ...
          <FormControl isInvalid={touched.desStation && errors.desStation}>
            <FormLabel>Destination Station</FormLabel>
            <Input
              name="desStation"
              type="text"
              value={values.desStation}
              onChange={handleChange}
              onBlur={handleBlur}
            />
            {touched.desStation && errors.desStation && (
              <Text color="red.500" fontSize="sm">
                {errors.desStation}
              </Text>
            )}
