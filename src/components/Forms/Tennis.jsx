...
function Tennis() {
  ...
  const sendData = async (ticketData, index) => {
    ...
    try {
      await dispatch(addSportTicketThunk({ ...ticketData, sportType: "tennis", userId })).unwrap();
      ...
      {matches.map((ticket, index) => (
        ...
        >
          ...
          <VStack align="start" mt={4} spacing={2}>
            <HStack><Text fontWeight="bold">Date:</Text><Text>{ticket.date}</Text></HStack>
          ...
          <Button mt={4} colorScheme="teal" isDisabled={loadingStates[index]} onClick={() => sendData(ticket, index)} size={{ base: "md", sm: "lg" }}>
